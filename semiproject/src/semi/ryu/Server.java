...
//            BufferedWriter bw = chatWlist.get(address); 
   ...
   static class Game{
      //101번
      static void codeSend(String myCounter, String ip, String utf) throws Exception{
         try {
      ...
      //103번
      static void codeHit(String myCounter, String ip) {
      ...
      static void codeWrong(String myCounter, String ip) {
         try {
              DataOutputStream dos = nickDos.get(myCounter);
              int send = 104;
              dos.writeInt(send);
...
//         wlist.put(닉네임, bw);
   ...
   //                        chatWlist.put(msg,bw); // 닉네임으로 귓속말이 되도록 설정
               ...
               // 101번 내 코드를 상대에게 전달
               String myCounter = myCounterpart.get(ip);
         ...
         //            gameList.put(ip,oos); // 게임 대상에 오브젝트 전송통로 연결
                  ...
                  // 넘버를 그대로 103을 넘겨주면 됨
                  Game.codeHit(myCounter, ip);
               } else if(num == 104) {
                  Game.codeWrong(myCounter, ip);
